  ...
  if (!room) {
    ...
    socket.on("connect", () => {
      ...
      socket.on("emojiReaction", (data) => {
        h.showEmojiReaction(data.emoji);
      });
    });
    ...
    function sendEmoji(emoji) {
      ...
      //emit chat message
      socket.emit("emojiReaction", data);
      h.showEmojiReaction(emoji);
      //add localchat
      // h.addChat(data, "local");
    }
    ...
    document.querySelectorAll(".emoji").forEach((emojiElement) => {
      emojiElement.addEventListener("click", (e) => {
        const emoji = e.target.getAttribute("data-emoji"); // Get the emoji from the data attribute
        sendEmoji(emoji); // Call the function with the emoji
      });
    });
